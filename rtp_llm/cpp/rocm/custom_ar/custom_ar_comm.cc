...
void CustomAllReduceComm::allGather(torch::Tensor& input_tensor, torch::Tensor& output_tensor) {
    const int last_dim_size = static_cast<int>(input_tensor.size(input_tensor.dim() - 1));
    const int gather_dim    = 0;
    if (at::hip::currentStreamCaptureStatusMayInitCtx() != at::hip::CaptureStatus::None) {
        aiter::all_gather_reg(fa_, input_tensor, output_tensor, last_dim_size, gather_dim);
    } else {
        aiter::all_gather_unreg(fa_, input_tensor, buffer_, output_tensor, last_dim_size, gather_dim);
    }
}
